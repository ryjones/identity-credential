...
import com.android.identity.securearea.cloud.CloudSecureAreaAttestationExtension
...
fun isCloudKeyAttestation(chain: X509CertChain): Boolean {
    return chain.certificates.first()
        .getExtensionValue(OID.X509_EXTENSION_MULTIPAZ_CSA_KEY_ATTESTATION.oid) != null
}
...
fun validateCloudKeyAttestation(
    ...
    val extensionDerEncodedString = leafX509Cert.getExtensionValue(OID.X509_EXTENSION_MULTIPAZ_CSA_KEY_ATTESTATION.oid)
        ?: throw IllegalStateException(
            "No attestation extension at OID ${OID.X509_EXTENSION_MULTIPAZ_CSA_KEY_ATTESTATION.oid}")

    val attestation = CloudSecureAreaAttestationExtension.decode(ByteString(extensionDerEncodedString))
