...
class CloudSecureAreaServer(
    ...
    data class RegisterState(
        ...
        var deviceCheckClientId: String? = null,
    ...
    private fun doRegisterRequest0(request0: RegisterRequest0,
        ...
        state.deviceCheckClientId = Random.Default.nextBytes(32).toHex()
        state.cloudChallenge = Random.Default.nextBytes(32)
        val response0 = RegisterResponse0(
            deviceCheckClientId = state.deviceCheckClientId!!,
        ...
        try {
            request1.deviceAttestation.validate(
                DeviceAttestationValidationData(
                    clientId = state.deviceCheckClientId!!,
            ...
            request1.deviceAttestation.validateAssertion(request1.deviceAssertion)
        } catch (e: Throwable) {
            Logger.w(TAG, "$remoteHost: RegisterRequest1: DeviceCheck did not validate", e)
            e.printStackTrace()
            return Pair(403, e.message!!.toByteArray())
        }
        state.deviceAttestation = request1.deviceAttestation

        // Android devices will provide key attestations for DeviceBindingKey. Check this.
        if (state.deviceAttestation is DeviceAttestationAndroid) {
            try {
                validateAndroidKeyAttestation(
                    chain = request1.deviceBindingKeyAttestation!!,
                    nonce = ByteString(state.cloudChallenge!!),
        ...
        val cloudBindingKeyAttestation = X509CertChain(
            listOf(
                X509Cert.Builder(
                    ...
                    .addExtension(
                        oid = OID.X509_EXTENSION_MULTIPAZ_CSA_KEY_ATTESTATION.oid,
                        critical = false,
                        value = CloudSecureAreaAttestationExtension(
    ...
    private suspend fun doCreateKeyRequest1(
        ...
        // Android devices will provide key attestations for LocalKey. Check this.
        if (e2eeState.context!!.deviceAttestation is DeviceAttestationAndroid) {
            try {
                validateAndroidKeyAttestation(
        ...
        val attestationCert = X509Cert.Builder(
            ...
            .addExtension(
                ...
                value = CloudSecureAreaAttestationExtension(
                    ...
                    passphraseRequired = state.passphraseRequired,
                    userAuthenticationRequired = state.userAuthenticationRequired,
                    userAuthenticationTypes = state.userAuthenticationTypes.toCloudSecureAreaUserAuthType()
                ).encode().toByteArray()
            )
            .build()
